...
import { getCertificates } from 'apiSdk/certificates';
import { CertificateInterface } from 'interfaces/certificate';

import { CertificateListPage as ShowCaseList } from 'pages/certificates';
...
function DashboardPage() {
  ...
  const certificateStat = useSWR<PaginatedInterface<CertificateInterface>>(
    () => `/certificates`,
    () => getCertificates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Certificates',
      result: certificateStat?.data?.totalCount,
      isLoading: certificateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
